...
def gendata(data_path, label_path, out_path, part='train', config='27'):
    ...
    for i, line in enumerate(label_file.readlines()):
        print("\r(reading: {})".format(i + 1), end ='')
        line = line.strip()
        line = line.split(',')
        data_name = os.listdir(data_path)[i]
        sample_names.append(line[0])
        data.append(os.path.join(data_path, data_name))
        #print(line[1])
    ...
    for i, data_path in enumerate(data):
        print("\r(processing: {} / {})".format(i + 1, len(data)), end ='')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='/dataset/KETI_SignLanguage/Keypoints-removal') #'train_npy/npy', 'va_npy/npy'
    parser.add_argument('--label_path', default='/dataset/KETI_SignLanguage/Annotation/label/label_KETI_train_all.csv') # 'train_labels.csv', 'val_gt.csv', 'test_labels.csv'
