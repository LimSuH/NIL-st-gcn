...
def video_detect(whole_stop, videos, dataset, dataname):
    ...
    # read video
        ...
        name = '/dataset/' + dataname + '/removal/' + path +'.mp4'
        print(name)
        fps = video.fps
        size = (video.width, video.height)
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        videoWriter = cv2.VideoWriter(name, fourcc, fps, size)
...
def check_tedency(datasets, max_frame):

    idx = 0
    whole_stop = np.zeros(max_frame + 1)
...
if __name__ == '__main__':
    ...
    KETI_trend = check_tedency(KETI_DIR, KETI_max)
    #AUTSL_trend = check_tedency(AUTSL_DIR, AUTSL_max)
