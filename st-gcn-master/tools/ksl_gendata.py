...
class KETI_gendata(IO):
    ...
    def start(self):
        ...
        val_data_out_path = self.arg.data + '/skeleton/val_data.npy'

        label_path = self.arg.data + '/raw/label.json'
        label_out_path = self.arg.data + '/skeleton/label.pkl'
        val_label_out_path = self.arg.data + '/skeleton/val_label.pkl'
        ...
        fp_train = open_memmap(
                ...
                dtype='float32',
                mode='w+',
                shape=(train_num, 3, max_frame, 18, num_person_out))

        fp_val = open_memmap(
                val_data_out_path,
                dtype='float32',
                mode='w+',
                shape=(data_num - train_num, 3, max_frame, 18, num_person_out))
        ...
        for i, video_name in enumerate(file_list):
            ...
            if data_numpy is not None:
                if i < train_num:
                    fp_train[i, :, 0:data_numpy.shape[1], :, :] = data_numpy
                else:
                    fp_val[i - train_num, :, 0:data_numpy.shape[1], :, :] = data_numpy
            else:
                print(i + 1)
                continue
        ...
        with open(label_out_path, 'wb') as f:
            pickle.dump((file_list[:train_num], list(label[:train_num])), f)
        
        with open(val_label_out_path, 'wb') as f:
            pickle.dump((file_list[train_num:], list(label[train_num:])), f)
